...
void interleaveQueue(struct Queue* q)
{
    ...
    //original queuela split it into two halves
    int halfSize = q->size / 2;
    for (int i = 0; i < halfSize; i++) {
        enQueue(queue1, front(q));
        deQueue(q);
    }
    while (!isQueueEmpty(q)) {
        enQueue(queue2, front(q));
        deQueue(q);
    }

    // Interleave the elements of the two queues back into the original queue
    while (!isQueueEmpty(queue1) || !isQueueEmpty(queue2)) {
        if (!isQueueEmpty(queue1)) {
            enQueue(q, front(queue1));
            deQueue(queue1);
        }

        if (!isQueueEmpty(queue2)) {
            enQueue(q, front(queue2));
            deQueue(queue2);
        }
    }
    free(queue1);
